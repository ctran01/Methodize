...
const TaskDetailsProjectForm = ({ task, closeModal, open, setTasks }) => {
  ...
  const onSubmit = async ({ name, due_date, description, completed }) => {
    ...
    try {
      ...
      const res = await apiServer.get(`/tasklist/${task.tasklist_id}/tasks`);
      setTasks(res.data);
      closeModal();
    } catch (err) {
      console.log(err);
...
export default TaskDetailsProjectForm;
